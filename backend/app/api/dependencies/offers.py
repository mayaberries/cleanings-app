...
async def get_offer_for_service_from_user(
    *, user: UserInDB, service: ServiceInDB, offers_repo: OffersRepository
) -> OfferInDB:
    offer = await offers_repo.get_offer_for_service_from_user(
...
async def get_offer_for_service_from_user_by_path(
    ...
    return await get_offer_for_service_from_user(
        user=user, service=service, offers_repo=offers_repo
    )


async def list_offers_for_service_by_id_from_path(
    service: ServiceInDB = Depends(get_service_by_id_from_path),
    offers_repo: OffersRepository = Depends(get_repository(OffersRepository))
) -> List[OfferInDB]:
    return await offers_repo.list_offers_for_service(service=service)
...
async def check_offer_create_permissions(
    ...
    if user_owns_service(user=current_user, service=service):
        raise HTTPException(
            ...
            detail="Users are unable to create offers for service jobs they own.",
        )
    if await offers_repo.get_offer_for_service_from_user(service=service, user=current_user):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Users aren't allowed to create more than one offer for a service job."
        )
...
def check_offer_get_permissions(
    ...
    if not user_owns_service(user=current_user, service=service) and offer.user_id != current_user.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
def check_offer_acceptance_permissions(
    ...
    if not user_owns_service(user=current_user, service=service):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only the owner of the service may accept offers."
        )
    ...
    if "accepted" in [o.status for o in existing_offers]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="That service job already has an accepted offer."
        )


async def get_offer_for_service_from_current_user(
    current_user: UserInDB = Depends(get_current_active_user),
    service: ServiceInDB = Depends(get_service_by_id_from_path),
    offers_repo: OffersRepository = Depends(get_repository(OffersRepository)),
) -> OfferInDB:
