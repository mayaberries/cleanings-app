...
Publishable-style keys (`pk_live_...` / `pk_test_...`) a clinic admin issues
to authenticate the public booking surface. A clinic can hold several at
once so a leaked or rotated key can be revoked with zero embed downtime.
...
Interactive docs land at `/docs` once the server is up.
